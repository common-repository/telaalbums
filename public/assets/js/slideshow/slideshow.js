telaalbums.slideshow = {
	...
	jssor_1_options: null,
	jssor_1_slider: null,
	
	init: function( id ) {
		
		let _this = this;
		_this.jssor_1_options = {
			$AutoPlay: 1,
			$FillMode: 5,
			$SlideshowOptions: {
				$Class: $JssorSlideshowRunner$,
				$Transitions: _this.jssor_1_SlideshowTransitions,
				$TransitionsOrder: 1
			},
		...
		_this.jssor_1_slider = new $JssorSlider$( id, _this.jssor_1_options );
		_this.ScaleSlider();
		$Jssor$.$AddEvent( window, "load", _this.ScaleSlider );
		$Jssor$.$AddEvent( window, "resize", _this.ScaleSlider );
		$Jssor$.$AddEvent( window, "orientationchange", _this.ScaleSlider );
	},
	
	ScaleSlider: function() {
		
		let _this = telaalbums.slideshow;
		var refSize = _this.jssor_1_slider.$Elmt.parentNode.clientWidth;
		if ( refSize ) {
			
			refSize = Math.min( refSize, 600 );
			_this.jssor_1_slider.$ScaleWidth(refSize);
		} else {
